  ...
  factory: (require) => {
    ...
    function ChatPanel({ sessions, workspaces, modelDirectories, notePath, noteContent, insertRequest }) {
      ...
      useEffect(() => {
        ...
        setAttachments((value) => [...value, insertRequest]);
      }, [insertRequest?.id]);
      const send = async () => {
        ...
        setSending(true);
        setError("");
        const attachedContext = attachments.map((attachment) => `\n\n${attachment.kind === "directory" ? "Obsidian 文件夹" : "Obsidian 文件"}：${attachment.path}\n\n${attachment.content}`).join("");
        const noteContext = notePath && noteContent !== null
          ? `\n\n当前 Obsidian 笔记：${notePath}\n\n笔记内容：\n${noteContent.slice(0, 16000)}`
          : "";
        const context = `${attachedContext}${noteContext}\n\n用户问题：${question}`;
        try {
