...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      "._ow_wysiwyg h1:hover,._ow_wysiwyg h2:hover,._ow_wysiwyg h3:hover,._ow_wysiwyg p:hover,._ow_wysiwyg ._ow_blank:hover{background:color-mix(in srgb,#7c65d7 6%,transparent)}",
      "._ow_editline{display:block;box-sizing:border-box;width:calc(100% + 8px);min-height:30px;margin:0 0 12px -4px;padding:5px 7px;border:1px solid color-mix(in srgb,#7c65d7 50%,var(--dsw-alias-border-l2));border-radius:7px;background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary);outline:0;white-space:pre-wrap;word-break:break-word;font:inherit;line-height:1.8;box-shadow:0 0 0 2px color-mix(in srgb,#7c65d7 10%,transparent)}",
    ...
    function EditableMarkdown({ markdown, onChange }) {
      ...
      const startEditing = (index) => {
        ...
        setDraftLine(editableSource(lines[index]));
      };
      ...
      const updateLine = (next) => {
        ...
        nextLines[editingLine] = normalizeEditableText(next);
        onChange(nextLines.join("\n"));
        if (/^(#{1,6})\s*.+?\s*\1\s*$/.test(next.trim()) && !next.includes("\n")) {
      ...
      const finishEditing = () => {
        if (editingLine === null) return;
        const nextLines = [...lines];
        nextLines[editingLine] = normalizeEditableText(draftLine);
        onChange(nextLines.join("\n"));
        setEditingLine(null);
      };

      return React.createElement("article", { className: "_ow_wysiwyg" }, lines.map((line, index) => editingLine === index
        ? React.createElement("div", {
          ...
          onInput: (event) => updateLine(event.currentTarget.innerText),
      ...
      return React.createElement("div", {
        ...
        React.createElement("main", { className: "_ow_center" },
          React.createElement("div", { className: "_ow_editorbar" },
            ...
            dirty && React.createElement("span", { className: "_ow_dirty" }, saving ? "自动保存中…" : "等待自动保存")),
