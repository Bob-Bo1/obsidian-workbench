...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      "._ow_wysiwyg{box-sizing:border-box;min-height:100%;max-width:900px;margin:0 auto;padding:34px 48px 70px;font-size:15px;line-height:1.8}",
      "._ow_wysiwyg h1,._ow_wysiwyg h2,._ow_wysiwyg h3{font-family:var(--ds-font-family,Inter,system-ui,sans-serif);line-height:1.3;margin:0 0 18px;color:var(--dsw-alias-label-primary);cursor:text;border-radius:6px;padding:2px 4px;margin-left:-4px}",
      "._ow_wysiwyg h1{font-size:28px}._ow_wysiwyg h2{font-size:22px;margin-top:28px}._ow_wysiwyg h3{font-size:18px;margin-top:24px}",
      "._ow_wysiwyg p{margin:0 0 14px;white-space:pre-wrap;cursor:text;border-radius:6px;padding:2px 4px;margin-left:-4px}",
      "._ow_wysiwyg ._ow_blank{height:18px;margin:0;cursor:text;border-radius:6px}",
      "._ow_editline{display:block;box-sizing:border-box;width:calc(100% + 8px);min-height:30px;margin:0 0 12px -4px;padding:2px 4px;border:0;border-radius:0;background:transparent;color:var(--dsw-alias-label-primary);outline:0;white-space:pre-wrap;word-break:break-word;font:inherit;line-height:1.8;box-shadow:none}",
      "._ow_editline:empty::before{content:'输入 Markdown…';color:var(--dsw-alias-label-tertiary)}",
      "._ow_editline[data-kind=title]{font-size:28px;font-weight:700;line-height:1.3}",
      "._ow_editline[data-kind=title]:empty::before{content:'输入主题…'}",
      ...
      "@media(max-width:960px){._ow_overlay{grid-template-columns:220px 1px minmax(280px,1fr)}._ow_resize[data-side=right],._ow_right{display:none}._ow_wysiwyg{padding-left:24px;padding-right:24px}._ow_preview{padding-left:24px;padding-right:24px}}",
    ...
    function editableSource(line) {
      return String(line || "");
    }

    function normalizeEditableLine(line) {
      const value = String(line || "").replace(/\u00a0/g, " ").replace(/\s+$/, "");
      const wrapped = value.match(/^(#{1,6})\s*(.*?)\s*\1\s*$/);
      if (wrapped && wrapped[2].trim()) return `${wrapped[1]}${wrapped[2].trim()}${wrapped[1]}`;
      return value;
    }

    function normalizeEditableText(text) {
      return String(text || "").split(/\r?\n/).map(normalizeEditableLine).join("\n");
    }

    function previewLine(line, key, onEdit) {
      const parts = String(line).split(/(\[\[[^\]]+\]\]|##[^#\n]+##)/g);
      return React.createElement("p", { key, onClick: () => onEdit(key) }, parts.map((part, index) => part.startsWith("[[")
        ? React.createElement("span", { className: "_ow_wikilink", key: `${key}-${index}` }, part)
        : /^##[^#\n]+##$/.test(part)
          ? React.createElement("strong", { key: `${key}-${index}` }, part.slice(2, -2))
        : part));
    }

    function renderPreviewLine(line, key, onEdit) {
      if (line.trim() === "") return React.createElement("div", { className: "_ow_blank", key, onClick: () => onEdit(key) });
      if (/^###\s+/.test(line)) return React.createElement("h3", { key, onClick: () => onEdit(key) }, line.replace(/^###\s+/, ""));
      if (/^##\s+/.test(line)) return React.createElement("h2", { key, onClick: () => onEdit(key) }, line.replace(/^##\s+/, ""));
      if (/^#\s+/.test(line)) return React.createElement("h1", { key, onClick: () => onEdit(key) }, line.replace(/^#\s+/, ""));
      if (/^```/.test(line)) return React.createElement("p", { key, className: "_ow_codehint", onClick: () => onEdit(key) }, line);
      return previewLine(line, key, onEdit);
    }
    ...
    function EditableMarkdown({ markdown, onChange }) {
      const lines = String(markdown || "").split(/\r?\n/);
      const [editingLine, setEditingLine] = useState(null);
      const [editingKind, setEditingKind] = useState("body");
      ...
      useEffect(() => {
        if (editingLine === null || !editRef.current) return;
      ...
      const startEditing = (index) => {
        ...
        const titleMode = lines.length === 1 && !lines[0].trim() && index === 0;
        const source = titleMode ? "" : editableSource(lines[index]);
        setEditingLine(index);
        setEditingKind(titleMode ? "title" : "body");
        draftRef.current = source;
        setDraftLine(source);
      };

      const storedLine = (value) => editingKind === "title"
        ? (value.trim() ? `# ${value.trim()}` : "")
        : normalizeEditableText(value);

      const updateLine = (next) => {
        if (editingLine === null) return;
        draftRef.current = next;
        const nextLines = [...lines];
        nextLines[editingLine] = storedLine(next);
        onChange(nextLines.join("\n"));
        if (editingKind === "body" && /^(#{1,6})\s*.+?\s*\1\s*$/.test(next.trim()) && !next.includes("\n")) {
          window.requestAnimationFrame(() => setEditingLine((value) => value === editingLine ? null : value));
        }
      };
      ...
      const finishEditing = () => {
        if (editingLine === null) return;
        const nextLines = [...lines];
        nextLines[editingLine] = storedLine(draftRef.current);
        onChange(nextLines.join("\n"));
        setEditingLine(null);
      };
      ...
      const insertLineAfter = () => {
        ...
        const nextLines = [...lines];
        nextLines[editingLine] = storedLine(draftRef.current);
        nextLines.splice(editingLine + 1, 0, "");
      ...
      const clearAll = () => {
        ...
        draftRef.current = "";
        setDraftLine("");
        setEditingKind("title");
        onChange("");
        setEditingLine(0);
      };

      return React.createElement("article", {
        className: "_ow_wysiwyg",
        ref: articleRef,
        onMouseDown: () => { allSelectedRef.current = false; },
        onClick: () => {
          if (editingLine === null && lines.length === 1 && !lines[0].trim()) startEditing(0);
        }
      }, lines.map((line, index) => editingLine === index
        ? React.createElement("div", {
          className: "_ow_editline",
          key: index,
          ref: editRef,
          "data-kind": editingKind,
          contentEditable: true,
          suppressContentEditableWarning: true,
          onCompositionStart: () => { composingRef.current = true; },
          onCompositionEnd: (event) => {
            composingRef.current = false;
            updateLine(editableText(event.currentTarget));
          },
          onInput: (event) => {
            if (!composingRef.current) updateLine(editableText(event.currentTarget));
          },
          onBlur: () => { if (!skipBlurRef.current) finishEditing(); },
          onKeyDown: (event) => {
            ...
            if ((allSelectedRef.current || (selectionText && selectionText === articleText)) && (event.key === "Backspace" || event.key === "Delete")) {
              event.preventDefault();
              clearAll();
              return;
            }
        ...
        : renderPreviewLine(line, index, startEditing)));
    }
      ...
      return React.createElement("div", {
        ...
        React.createElement("main", { className: "_ow_center" },
          ...
          React.createElement("div", { className: "_ow_editor" },
            ...
            note && !note.image && React.createElement(EditableMarkdown, { markdown: editor, onChange: setEditor })),
