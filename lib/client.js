...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      "._ow_live{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);height:100%;min-height:100%}",
      "._ow_liveinput{min-width:0;min-height:100%;border-right:1px solid var(--dsw-alias-border-l1)}",
      "._ow_liveoutput{max-width:none;margin:0;min-width:0;overflow:auto}",
    ...
    function ChatPanel({ sessions, workspaces, modelDirectories, notePath, noteContent, insertRequest }) {
      ...
      useEffect(() => {
        if (!insertRequest?.id || typeof insertRequest.text !== "string") return;
        setDraft((value) => value.trim() ? `${value}\n\n${insertRequest.text}` : insertRequest.text);
      }, [insertRequest?.id]);
      const send = async () => {
    ...
    function Workbench({ onClose, sessions, workspaces, modelDirectories }) {
      ...
      const addToChat = async (entry) => {
        ...
        try {
          ...
          setChatInsertRequest({ id: Date.now(), text: `请参考以下 Obsidian 内容：\n\n${payload.content}\n\n${suffix}请结合这些内容回答我的问题：` });
        ...
        React.createElement("main", { className: "_ow_center" },
          React.createElement("div", { className: "_ow_editorbar" },
            ...
            note && !note.image && React.createElement("div", { className: "_ow_mode" },
              ...
              React.createElement("button", { type: "button", "data-active": view === "live", onClick: () => setView("live") }, "实时"),
          ...
          React.createElement("div", { className: "_ow_editor" },
            ...
            note && !note.image && view === "live" && React.createElement("div", { className: "_ow_live" },
              React.createElement("textarea", { className: "_ow_textarea _ow_liveinput", value: editor, onChange: (event) => setEditor(event.target.value), spellCheck: false }),
              React.createElement("article", { className: "_ow_preview _ow_liveoutput" }, markdownPreview(editor))),
