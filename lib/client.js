...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      "._ow_movebar{display:flex;align-items:center;gap:5px;margin:0 4px 7px;padding:6px 7px;border-radius:8px;background:color-mix(in srgb,#7c65d7 10%,var(--dsw-alias-interactive-bg-hover));color:var(--dsw-alias-label-secondary);font-size:11px;line-height:1.35}",
      "._ow_movebar span{min-width:0;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}",
      ...
      "._ow_treeentry[data-drop-target=true]{outline:1px dashed #7c65d7;outline-offset:-2px;background:color-mix(in srgb,#7c65d7 12%,var(--dsw-alias-interactive-bg-hover))}",
    ...
    function TreeDirectory({ entry, selected, onOpen, revision, movingEntry, onMoveStart, onMoveTarget, onFolderFocus, onContextMenu }) {
      ...
      return React.createElement(React.Fragment, null,
        React.createElement("button", {
          ...
          "data-drop-target": Boolean(movingEntry && movingEntry.path !== entry.path),
          ...
          onDragOver: (event) => { if (movingEntry || event.dataTransfer.types.includes("text/plain")) event.preventDefault(); },
        ...
        open && React.createElement("div", { className: "_ow_treechildren" },
          error ? React.createElement("div", { className: "_ow_hint" }, error) : tree && React.createElement(TreeContents, { tree, selected, onOpen, revision, movingEntry, onMoveStart, onMoveTarget, onFolderFocus, onContextMenu }))
      );
    }

    function TreeFile({ entry, selected, onOpen, movingEntry, onMoveStart, onContextMenu }) {
      const timer = useRef(null);
      const longPressed = useRef(false);
      const dragged = useRef(false);
      const clearPress = () => {
    ...
    function TreeContents({ tree, selected, onOpen, revision, movingEntry, onMoveStart, onMoveTarget, onFolderFocus, onContextMenu }) {
      return React.createElement(React.Fragment, null,
        tree.dirs.map((entry) => React.createElement(TreeDirectory, { key: entry.path, entry, selected, onOpen, revision, movingEntry, onMoveStart, onMoveTarget, onFolderFocus, onContextMenu })),
        tree.files.map((entry) => React.createElement(TreeFile, { key: entry.path, entry, selected, onOpen, movingEntry, onMoveStart, onContextMenu }))
      );
    }

    function VaultTree({ selected, onOpen, revision, movingEntry, onMoveStart, onMoveTarget, onFolderFocus, onContextMenu }) {
      const [tree, setTree] = useState(null);
      const [error, setError] = useState("");
      ...
      return React.createElement(TreeContents, { tree, selected, onOpen, revision, movingEntry, onMoveStart, onMoveTarget, onFolderFocus, onContextMenu });
    }
    ...
    function Workbench({ onClose, sessions, modelDirectories }) {
      ...
      const submitName = async (event) => {
        ...
        else await createNoteAt(dialog.parent, name);
      };
      ...
      const onKeyDown = (event) => {
        if (event.key === "Escape") {
          if (nameDialog) setNameDialog(null);
      ...
      return React.createElement("div", {
        ...
        nameDialog && React.createElement("div", { className: "_ow_dialogbackdrop", onPointerDown: () => setNameDialog(null) },
          React.createElement("form", { className: "_ow_dialog", onSubmit: submitName, onPointerDown: (event) => event.stopPropagation() },
            React.createElement("div", { className: "_ow_dialogtitle" }, nameDialog.kind === "folder" ? "新建文件夹" : "新建笔记"),
