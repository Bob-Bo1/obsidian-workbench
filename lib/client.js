  ...
  factory: (require) => {
    ...
    function EditableMarkdown({ markdown, onChange }) {
      ...
      const startEditing = (index) => {
        const source = editableSource(lines[index]);
      ...
      const finishEditing = () => {
        if (editingLine === null) return;
        const nextLines = [...lines];
        nextLines[editingLine] = normalizeEditableText(draftRef.current);
        onChange(nextLines.join("\n"));
        setEditingLine(null);
      };
      ...
      return React.createElement("article", { className: "_ow_wysiwyg" }, lines.map((line, index) => editingLine === index
        ? React.createElement("div", {
          ...
          onCompositionEnd: (event) => {
            ...
            updateLine(event.currentTarget.innerText);
          },
          onInput: (event) => {
            if (!composingRef.current) updateLine(event.currentTarget.innerText);
          },
